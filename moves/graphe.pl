...
generateMove(BoardInit, Player, BestMove) :-
									...
									asserta(idNoeud(max,-100,100,_,BoardInit,-100)),
                                    write('Bug1.5'),exploMiniMax(PossibleMoveList,Player,4,BoardInit),
									write('Bug1.6'),findall((Board,H),idNoeud(_,_,_,BoardInit,Board,H),L),
									write('Bug1.7'),meilleurBoard(0,L,BestBoard),
									write('Bug1.8'),listall((A,B,C,BoardInit,E,F),idNoeud(A,B,C,BoardInit,E,F),ListeIdNoeud),
...
miniMax(Player, 0,InBoard,Board) :-
												...
												modifNoeud(_,_,_,InBoard,Board,Heuristique).

												
/*Cas2 si on atteint un noeud*/										
miniMax(Player,Strate,_,Board) :- idNoeud(_,_,_,_,Board,_),
											 possibleMovesMiniMax(Board,Player,List),
											 sort(List,PossibleMoveList),
											 exploMiniMax(PossibleMoveList,Player,Strate,Board).

modifNoeud(MinOuMax,Alpha,Beta,InBoard,Board,H):- idNoeud(MinOuMax,Alpha,Beta,InBoard,Board,H),H>Alpha,retract(idNoeud(MinOuMax,Alpha,Beta,InBoard,Board,H)),
													  asserta(idNoeud(MinOuMax,Alpha,H,InBoard,Board,H)).
modifNoeud(_,_,_,_,_,_).
...
exploMiniMax([(Col1,Lin1,Col2,Lin2)|Res],Player,Strate,InBoard) :-    			write('Bug2.0'),idNoeud(MinOuMax,Alpha,Beta,_,InBoard,Value),
																				...
																				write('Bug2.11'),exploMiniMax(Res,Player,Strate,InBoard,ListePions).
