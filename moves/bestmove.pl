...
alphaBeta(Player, 0, Board, _, _, _, Value) :-
...
alphaBeta(Player, Frontier, Board, Alpha, Beta, Move, Value) :-
  Frontiere > 0, % On vérifie que la frontière est bien positive !
...
evaluerEtChoisir(Player, [Move|MoveList], Board, Frontier, Alpha, Beta, Record, BestMove) :-
  ...
  NValue is -Value,
  coupure(Player, Move, NValue, Frontier, Alpha, Beta, MoveList, Position, Record, BestMove).
...
coupure(Player, Move, Value, Frontier, Alpha, Beta, Moves, Position, _, BestMove) :-
  Alpha < Value, Value < Beta, !,
  evaluerEtChoisir(Player, Moves, Position, Frontier, Value, Beta, Move, BestMove).

coupure(Player, _, Value, Frontier, Alpha, Beta, Moves, Position, Record, BestMove) :-
  Value =< Alpha, !,
