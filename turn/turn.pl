...
mainLoop(InBoard, rouge, humain, J2) :-
  playTurn(InBoard, rouge, OutBoard),
  getCote(Cote, rouge), afficherPlateau(OutBoard, Cote),
  mainLoop(OutBoard, ocre, humain, J2), !.

mainLoop(InBoard, rouge, machine, J2) :-
  playTurnAI(InBoard, rouge, OutBoard),
  getCote(Cote, rouge), afficherPlateau(OutBoard, Cote),
  mainLoop(OutBoard, ocre, machine, J2), !.

mainLoop(InBoard, ocre, J1, humain) :-
  playTurn(InBoard, ocre, OutBoard),
  getCote(Cote, ocre), afficherPlateau(OutBoard, Cote),
  mainLoop(OutBoard, rouge, J1, humain), !.

mainLoop(InBoard, ocre, J1, machine) :-
  playTurnAI(InBoard, ocre, OutBoard),
  getCote(Cote, ocre), afficherPlateau(OutBoard, Cote),
  mainLoop(OutBoard, rouge, J1, machine).

setWinner(Colour) :-
  player(TypePlayer, Colour),
